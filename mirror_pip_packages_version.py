...
if __name__ == '__main__':
    ...
    for line in lines:
        if not line.startswith("#"):
            if "==" in line:
                (_pkg, _ver) = line.strip('\n').split("==")
                pkgs.append((_pkg, _ver))
    ...
    for (pkg, _ver) in pkgs:
        ...
        urls = pkg_url_map[pkg] 
        pkg_ver_pattern = "{}-{}".format(pkg, _ver)
        mod_pkg = pkg.replace('-','_')
        mod_pkg_ver_pattern = "{}-{}".format(mod_pkg, _ver)

        for url in urls:
            if (pkg_ver_pattern in url) or (mod_pkg_ver_pattern in url):
