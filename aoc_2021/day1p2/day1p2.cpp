...
int main(int argc, const char * argv[]) {
    ...
    int64_t loc = -1;
    int incr = 0;
    for (size_t cnt = 0; cnt < inpList.size(); cnt++) {
        if (cnt+2 == inpList.size()) break;

        int64_t consSum = inpList[cnt] + inpList[cnt+1] + inpList[cnt+2];
        if (loc != -1 && consSum > loc) {
            incr += 1;
        }
        loc = consSum;
    }

    cout << format("Number of time increases: {}", incr) << endl;
